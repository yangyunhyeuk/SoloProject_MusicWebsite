    ...
    <script type="text/javascript">
        window.onload = () => {
            ...
            if (part === 1) {
                ...
                prevElement.href = "/board/list?page=" + ((part - 1) * pageCount) + "&size=" + size;
            }

            if (part === totalPartCount) {
                nextElement.style.display = "none";
            } else {
                prevElement.href = "/board/list?page=" + (part * pageCount + 1) + "&size=" + size;
            }
            ...
            for (let i = (part - 1) * pageCount; i < part * pageCount && i < totalPageCount; ++i) {
                pagesElement.innerHTML += (page === i ? '<a class="activePage"' : "<a") + ' href="/board/list?page=' + i + '&size=' + size + '">' + (i + 1) + "</a>";
            }
        };
    </script>
    ...
    <div class="container">
        ...
        <div class="row">
            <c:if test="${sessionScope.mid eq 'admin'}">
                <div>
                    <a href="/writepost">
                        <button class="primary-button">글 작성하기</button>
                    </a> <br> <br> <br>
                </div>
            </c:if>
            <c:choose>
                <c:when test="${not empty list}">
                    <c:forEach items="${list}" var="vo">
                        <div class="col-md-4">
                            ...
                            <div class="post">
                                <%--
                                <a class="post-img" href="/board/post?bnum=${vo.bnum}"> <img
                                        src="/getImage?filename=${vo.filename}" alt=""></a>
                                --%>
                                <a class="post-img" href="/board/post?page=${page}&size=${size}&bnum=${vo.bnum}"> <img
                                        src="/getImage?filename=${vo.filename}" alt=""></a>

                            </div>
                            <div class="section-title">
                                <h2 class="title">${vo.btitle}</h2>
                            </div>
                            <!-- /post -->
                        </div>
                    </c:forEach>
